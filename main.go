...
func main() {
	...
	allPRs := getPrs(lastPage.Data.Repository.PullRequests.PageInfo.StartCursor, prs)

	filteredPrs := filterPRs(allPRs)

	fmt.Println("PR,reviewers,opened,merged")
	// fmt.Println("Number of PR's closed in August: ", len(filteredPrs))
	for _, pr := range filteredPrs {

		reviewers := ""

		for k, _ := range pr.Reviewers {

			reviewers += k + " | "

		}

		fmt.Printf("%d,%d,%v,%v\n", pr.Id, len(pr.Reviewers), pr.CreatedAt.Format("2006-01-02"), pr.MergedAt.Format("2006-01-02"))
	}
